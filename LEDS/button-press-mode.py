...
Cycles through 19 interactive animation modes using a physical hardware button.
Provides real-time keyboard adjustments and a terminal dashboard.

Hardware Requirements:
- Raspberry Pi
- RGB LED (Common Cathode)
- Tactile Push Button
- GPIO Mapping: RED=19, GREEN=13, BLUE=18, BUTTON=27
...
class RGBController:
    """Manages the low-level hardware interface for PWM color control and Button inputs."""
    def __init__(self, pin_r: int = 19, pin_g: int = 13, pin_b: int = 18, pin_btn: int = 27, freq: int = 2000):
        self.pin_r = pin_r
        self.pin_g = pin_g
        self.pin_b = pin_b
        ...
        if GPIO_AVAILABLE:
            ...
            # Setup Button Input with Pull-Down resistor (Active HIGH)
            GPIO.setup(self.pin_btn, GPIO.IN, pull_up_down=GPIO.PUD_DOWN)
...
class Animation:
    """Maintains time deltas, configurations, and vector calculations for 19 modes."""
...
class Menu:
    ...
    def display_dashboard(self, mode: int, fps: float, freq: int, brightness: float, 
                          rgb: tuple, hex_color: str, duty: tuple, speed: float):
        """Overwrites parameters cleanly from row 0 using ANSI vector offsets."""
        sys.stdout.write("\033[H")
        dashboard = (
            f"====================================================\n"
            f"          HARDWARE CONTROL TELEMETRY ACTIVE         \n"
            f"====================================================\n"
            f"Current Mode:        [{mode:02d}] {self.MODES[mode-1]:<25}\n"
            f"Mode Control:        Physical Button (GPIO 27)      \n"
...
class App:
    ...
    def run(self):
        ...
        try:
            ...
            frame_count = 0
            current_fps = 0.0
            last_btn_press_time = 0.0  # Used for non-blocking debouncing
            ...
            while self.running:
                ...
                # ---------------------------------------------------------
                # PHYSICAL BUTTON INPUT HANDLING (Non-Blocking)
                # ---------------------------------------------------------
                if GPIO_AVAILABLE:
                    # Check if button is pressed (HIGH due to PULL_DOWN)
                    if GPIO.input(self.controller.pin_btn) == GPIO.HIGH:
                        # Non-blocking 300ms debounce
                        if (now - last_btn_press_time) > 0.3:
